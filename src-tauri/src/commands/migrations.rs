...
async fn create_database_file(handle: &AppHandle) -> Result<(), String> {
    ...
    match fs::create_dir_all(dir_path) {
        Ok(_) => (),
        Err(error) => return Err(format!("Failed to create the database directory. Cause: {:?}", error))
    };

    let options = SqliteConnectOptions::new().filename(&path).create_if_missing(true);
    match SqlitePool::connect_with(options).await {
        Ok(_) => Ok(()),
        Err(error) => Err(format!("Failed to create the database file. Cause: {:?}", error))
    }
}
...
async fn database_exists(handle: &AppHandle) -> Result<bool, String> {
    ...
    match fs::exists(database_path) {
        Ok(exists) => Ok(exists),
        Err(error) => Err(format!("Failed to check if the database exists. Cause: {:?}", error))
    }
}
    ...
    match handle.path().resolve("db", BaseDirectory::AppConfig) {
        Ok(path) => {
            match path.into_os_string().into_string() {
                Ok(path) => Ok(path),
                Err(error) => Err(format!("Failed to get the database path. Cause: {:?}", error))
            }
        },
        Err(error) => Err(format!("Failed to get the database path. Cause: {:?}", error))
    }
...
pub async fn get_database_path(handle: &AppHandle) -> Result<String, String> {
    match handle.path().resolve("db/plauzible.db", BaseDirectory::AppConfig) {
        Ok(path) => {
            match path.into_os_string().into_string() {
                Ok(path) => Ok(path),
                Err(error) => Err(format!("Failed to get the database path. Cause: {:?}", error))
...
async fn run_migration(handle: &AppHandle, migration: &Migration<'_>) -> Result<(), String> {
    let pool = database::connect_to_database(handle).await?;
    println!("Running migration to version number: {}", migration.target_version);
    for sql in &migration.sql {
        println!("Running SQL: {}", sql);
        match sqlx::query(sql).execute(&pool).await {
            Ok(_) => (),
            Err(error) => return Err(format!("Failed to run migration. SQL: {}, Cause: {:?}", sql, error))
        }
    }
    Ok(())
}
...
pub async fn run_migrations(handle: &AppHandle) -> Result<(), String> {
    ...
    if exists {
        version = match database_version(handle).await {
            Ok(version) => version,
            Err(error) => return Err(format!("Failed to get the database version. Cause: {:?}", error))
        };
    } else {
